"""
BaselineEvaluate.py
针对 Entity_ZeroShot 图谱的RAG系统评估脚本（基于RAGAS + 实际图谱KG扩展）
...
OPENAI_API_KEY = "sk-a49a8e642f93450584a40fcae69c42ec"
...
class ZeroShotRAG:
    """
    针对 Entity_ZeroShot 和 RELATION_ZEROSHOT 的简化版 RAG 系统
    仅使用节点名称和关系类型构建上下文，忽略所有额外属性。
    """
    def __init__(self):
        ...
        self.VECTOR_INDEX_NAME = "vector_entity_zeroshot"
        self.NODE_LABEL = "Entity_ZeroShot"
        self.RELATION_TYPE = "RELATION_ZEROSHOT"
        ...
        for attempt in range(retries):
            try:
                response = openai.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system", "content": "你是一个有图谱分析专家。"},
...
if __name__ == "__main__":
    rag = ZeroShotRAG()
    evaluator = ZeroShotEvaluator(rag)

    QUESTION_DIR = "Question"
    RESULTS_DIR = "ZeroShot_Evaluation_Results"
    ...
    for file_path in topic_files:
        ...
        if avg_scores:
            ...
            # 保存详细结果
            detailed_file = os.path.join(RESULTS_DIR, f"{topic_name}_Zeroshot_detailed.json")
    ...
    if all_topics_results:
        output_file = os.path.join(RESULTS_DIR, "all_topics_zeroshot_evaluation.json")
