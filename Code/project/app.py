...
def get_baseline_graph(label_filter, topic_filter):
    ...
    with driver.session() as session:
        ...
        for record in node_result:
            ...
            node_map[node_id] = {
                "id": node_id,
                "label": props.get("name", "未知"),
                "title": f"名称: {props.get('name')}<br>话题: {record['topic_label']}",
        ...
        if nodes:
            ...
            for record in edge_result:
                ...
                edges.append({
                    "from": s.element_id,
                    "to": t.element_id,
                    "label": r.get("predicate", "未知"),
                    "title": f"关系: {r.get('predicate')}<br>话题: {r.get('topic')}",
                    "arrows": "to",
                })
...
def get_cognitive_graph(topic_filter):
    ...
    with driver.session() as session:
        ...
        for record in node_result:
            ...
            props = dict(n)
            node_id = n.element_id
            tooltip = f"名称: {props.get('name')}<br>类型: {props.get('type')}<br>立场: {props.get('dominant_stance')}<br>确定性: {props.get('dominant_certainty')}<br>出现次数: {props.get('total_count')}"
            node_map[node_id] = {
                "id": node_id,
                "label": props.get("name", "未知"),
                "title": tooltip,
        ...
        if nodes:
            ...
            for record in edge_result:
                ...
                t = record["t"]
                props = dict(r)
                tooltip = f"类型: {props.get('predicate')}<br>强度: {props.get('strength')}<br>置信度: {props.get('confidence')}<br>出现次数: {props.get('count')}"
                edges.append({
                    "from": s.element_id,
                    "to": t.element_id,
                    "label": props.get("predicate", "未知"),
                    "title": tooltip,
                    "arrows": "to",
                })
    return {"nodes": nodes, "edges": edges}
...
def api_rag_ask():
    if rag is None:
        return jsonify({'error': 'RAG 系统未初始化，请检查配置'}), 500
    data = request.get_json()
